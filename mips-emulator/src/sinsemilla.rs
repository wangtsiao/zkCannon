/// zero knowledge proof friendly hash function: sinsemilla implementation
...
mod tests {
    ...
    use crate::sinsemilla::HashDomain;
    ...
    #[test]
    fn hash_data() {
        const PERSONALIZATION: &str = "TestCRH";
        let q = HashDomain::new(PERSONALIZATION).Q;

        let data = [false; 500].map(
            |_| rand::random::<bool>()
        );

        let hasher = HashDomain::from_Q(q.into());
